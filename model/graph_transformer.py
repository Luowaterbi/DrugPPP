...
"""
Author: Atma
This code is based on MAT: https://github.com/ardigen/MAT
"""
import math, copy
...
### Model definition
def make_graph_transformer(d_atom, N=2, d_model=128, h=8, dropout=0.1,
                           ...
                           scale_norm=False, use_adapter=False, n_generator_layers=1):
...
def attention(query, key, value, adj_matrix, distances_matrix, edges_att,
    ...
    # print("debug: att: {} {}, \nadj: {} {}, \ndist: {} {}".format(p_attn.shape, p_attn, p_adj.shape, p_adj, p_dist.shape, p_dist))
...
class Embeddings(nn.Module):
    def __init__(self, d_model, d_atom, dropout):
        ...
        # print("debug: d_model, d_atom", d_model, d_atom)
        self.lut = nn.Linear(d_atom, d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        # print('debug: embedding input', x.shape)
