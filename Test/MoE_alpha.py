...
def top1(t):
    values, index = t.topk(k=1, dim=-1)  # [batch_size, token_num, 1]
    values, index = map(lambda x: x.squeeze(dim=-1), (values, index))  # [batch_size, token_num]
    return values, index
...
def cumsum_exclusive(t, dim=-1):
    ...
    padded_t = F.pad(t, (*pre_padding, 0, 1)).cumsum(dim=dim)
    return padded_t[(..., slice(-1, None), *pre_slice)]
...
class Top2Gating(nn.Module):
    def __init__(self, dim, num_gates, eps=1e-9, outer_expert_dims=tuple(), second_policy_train='random', second_policy_eval='random', second_threshold_train=0.2, second_threshold_eval=0.2, capacity_factor_train=1.25, capacity_factor_eval=2.):
        super().__init__()
    ...
    def forward(self, x, importance=None):
        *_, b, group_size, dim = x.shape
        ...
        raw_gates = torch.einsum('...bnd,...de->...bne', x, self.w_gating)  # [batch_size,token_num,gate_num]
        ...
        mask_1 = F.one_hot(index_1, num_gates).float()  # [batch_size, token_num, gate_num]
        ...
        combine_tensor = (gate_1[..., None, None] * mask_1_flat[..., None, None] * F.one_hot(index_1, num_gates)[..., None] * safe_one_hot(position_in_expert_1.long(), expert_capacity)[..., None, :] + gate_2[..., None, None] * mask_2_flat[..., None, None] * F.one_hot(index_2, num_gates)[..., None] * safe_one_hot(position_in_expert_2.long(), expert_capacity)[..., None, :])
...
class MoE(nn.Module):
    def __init__(self, dim, num_experts=16, hidden_dim=None, activation=nn.ReLU, second_policy_train='random', second_policy_eval='random', second_threshold_train=0.2, second_threshold_eval=0.2, capacity_factor_train=1.25, capacity_factor_eval=2., loss_coef=1e-2, experts=None):
        super().__init__()
    ...
    def forward(self, inputs, **kwargs):
        # b是batch_size, n是每个batch_size有几个token，d就是token的feature dim
...
class HeirarchicalMoE(nn.Module):
    def __init__(self, dim, num_experts=(4, 4), hidden_dim=None, activation=nn.ReLU, second_policy_train='random', second_policy_eval='random', second_threshold_train=0.2, second_threshold_eval=0.2, capacity_factor_train=1.25, capacity_factor_eval=2., loss_coef=1e-2, experts=None):
        super().__init__()
