                                      ...
                                      for att_block in ${att_block_lst[@]}; do
                                        for rn_dst in ${rn_dst_lst[@]}; do
                                          for cross_dst in ${cross_dst_lst[@]}; do
                                            # model names
                                            model_name=new_rnign_ddi.readout_${readout}.cr_dst_${cross_dst}.rn_dst_${rn_dst}.lambda_${lambda_att}_${lambda_dst}.bs_${train_batch_size}.ep_${epoch}.lr_${lr}_.d_model_${d_model}.inter_${interactor}_l_${inter_layer}_h_${inter_head}.enc_${encoder}_l_${enc_layer}_h_${enc_head}${debug}
